...
describe('CurrentWeatherComponent', () => {
  ...
  it('should eagerly load currentWeather in Yerevan from weatherService', () => {
    weatherServiceMock.getCurrentWeather.and.returnValue(of(fakeWeather));
    fixture.detectChanges();
    expect(component.current).toBeDefined();
    expect(component.current.city).toEqual('Yerevan');
    expect(component.current.temperature).toEqual(280.32);

    const debugEl = fixture.debugElement;
    const titleEl: HTMLElement = debugEl.query(By.css('span')).nativeElement;
    expect(titleEl.textContent).toContain('Yerevan');
  })
});
